...
public class DatabaseTesting_Webtable_Validation {
	...
	public void webTableVerify() throws SQLException {
		...
		/*
		 * collecting all data from webtable into ArrayList for later Comparison with DB
		 * arraylist, total row 6 and column 3 row index j = 1 (for column header row)
		 * so started from j = 2 Column index starts from i=1
		 * //*[@id='t01']/tbody/tr["+j+"]/td["+i+"]") this Dynamic Xpath take parameter
		 * and count and store value for total 18(3*6) table cell available
		 */

		List<String> webPage_data = new ArrayList<String>();
		for (int j = 2; j <= totalRow; j++) {
			for (int i = 1; i <= totalColumn; i++) {
				String row_data = driver.findElement(By.xpath("//*[@id='t01']/tbody/tr[" + j + "]/td[" + i + "]"))
						.getText();
