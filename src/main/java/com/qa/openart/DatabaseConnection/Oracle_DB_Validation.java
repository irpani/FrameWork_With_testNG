package com.qa.openart.DatabaseConnection;
...
/*DatabaseTesting_Webtable_Validation
This project is containing Database testing using Oracle 11g in Selenium Webdriver.

Connecting Oracle11g with Java and creatingSecure DB connection.
Performing user flow in fornt end using data coming from database directly via Query.
Post verify data in front end successfully updated or not against the database Table.
Asserting the contents, header and all other attributes.*/

public class Oracle_DB_Validation {
	...
	public static void main(String[] args) throws SQLException, IOException {
		...
		// fetching total row wise data| if row = 3 and column = 4 then total fields
		// will be =3*4 = 12

		for (int m = 0; m <= rowNumber * columnNumber; m++) {
			for (int i = 1; i <= rowNumber; i++) {
				System.out.println(i + " no Row : " + data.get(m) + "\t\t|  " + data.get(m + 1) + "\t\t|  "
						+ data.get(m + 2) + "\t|");
				++m;
				++m;
				++m;
		...
		/**
		 * remember after adding data to list, now data.get(0) = will return (1st
		 * Row)-1st Column data.get(1) = will return (1st Row)-2nd Column will print row
		 * wise from starting to end row
		 */
